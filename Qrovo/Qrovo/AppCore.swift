...
class QViewController: UIViewController {
    ...
    @discardableResult
    func requireSignedIn(message: String = "Please sign in to continue.") -> Bool {
        guard !QRepository.shared.isSignedIn else { return true }
        let alert = QAlertViewController(titleText: "Sign In Required", messageText: message, cancelTitle: nil, confirmTitle: "Sign In") { AppCoordinator.shared.showAuth() }
...
private func updateEULAActions(in view: UIView) {
    if let button = view as? UIButton, button.title(for: .normal) == "Publish" { button.setTitle("Agree", for: .normal) }
    view.subviews.forEach { updateEULAActions(in: $0) }
}

final class EULAViewController: QViewController {
    override func viewDidAppear(_ animated: Bool) { super.viewDidAppear(animated); updateEULAActions(in: view) }
