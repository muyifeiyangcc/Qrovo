...
final class ExploreViewController: RootPageViewController {
    ...
    override func viewDidLoad() { titleText = "Home"; super.viewDidLoad(); rebuild() }
    override func refreshData() { if !suppressRepositoryRefresh { rebuild() } }
    override func rebuild() { body.arrangedSubviews.forEach { $0.removeFromSuperview() }; heading("Home"); body.addArrangedSubview(guideCard()); body.addArrangedSubview(exploreSegmented()); body.addArrangedSubview(label("People near you tried", size: 18, weight: .semibold)); let posts = postsForSegment(); if posts.isEmpty { body.addArrangedSubview(emptyState("No field notes here yet", detail: "Try another view to find a new story.")) } else { posts.forEach { body.addArrangedSubview(postCard($0)) } } }
    ...
    private func openFirstPostByID(_ id: String) { guard requireSignedIn(message: "Please sign in to interact with this field note.") else { return }; guard let post = QRepository.shared.post(id) else { return }; push(FieldNoteViewController(post: post)) }
    private func openMore(for post: QPost) { guard requireSignedIn() else { return }; showMoreSheet(for: post.authorID, report: { [weak self] in guard let self else { return }; self.push(ReportViewController(userID: post.authorID)) }, block: { [weak self] in guard let self else { return }; QRepository.shared.block(userID: post.authorID); self.rebuild() }) }
    private func openTaskInDraw(_ post: QPost) { guard requireSignedIn() else { return }; guard let tabs = tabBarController as? MainTabBarController else { return }; tabs.presentPublish(for: post) }
