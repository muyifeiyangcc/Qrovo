...
final class DrawViewController: RootPageViewController {
    ...
    override func rebuild() {
        ...
        let notice = label("✓   Each draw task costs 120 coins · Task lasts 24 hours", size: 11, color: QTheme.secondaryText)
        ...
        let draw = button("120 coins Draw a task", height: 52)
...
final class PublishViewController: RootPageViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLLocationManagerDelegate, PHPickerViewControllerDelegate, UITextViewDelegate {
    ...
    private func publishPost() { guard let text = note.text?.trimmingCharacters(in: .whitespacesAndNewlines), text.count > 8 else { showMessage("Add a field note", "Tell the community what you found."); return }; let location = locationField.text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""; guard !location.isEmpty else { showMessage("Add a location", "Enter a location or use the location button."); return }; QRepository.shared.publish(title: selectedTask?.title ?? text.split(separator: ".").first.map(String.init) ?? text, body: text, location: location, assets: assets); showMessage("Published", "Your field note is ready to share.") { if let tabs = self.presentingViewController as? MainTabBarController { tabs.clearClaimedPublishTask(); self.dismiss(animated: true) { tabs.showExplore() } } else { self.tabBarController?.selectedIndex = 0 } } }
...
final class ProfileViewController: RootPageViewController {
    ...
    override func rebuild() { body.arrangedSubviews.forEach { $0.removeFromSuperview() }; let header = profileHeader(); body.addArrangedSubview(header); body.setCustomSpacing(18, after: header); let me = QRepository.shared.currentUser ?? QRepository.shared.user("me")!; body.addArrangedSubview(profileIdentity(me)); body.setCustomSpacing(17, after: body.arrangedSubviews.last!); body.addArrangedSubview(profileStats(me)); body.setCustomSpacing(18, after: body.arrangedSubviews.last!); body.addArrangedSubview(balanceBanner()); body.setCustomSpacing(12, after: body.arrangedSubviews.last!); body.addArrangedSubview(profileTabs()); body.setCustomSpacing(19, after: body.arrangedSubviews.last!); let checkIns = QRepository.shared.visiblePosts.filter { $0.authorID == me.id }; let posts = saved ? QRepository.shared.savedPosts : checkIns; if posts.isEmpty { body.addArrangedSubview(emptyState(saved ? "No saved check-ins yet" : "No check-ins yet")) } else { posts.prefix(3).forEach { body.addArrangedSubview(profilePost($0)) } } }
